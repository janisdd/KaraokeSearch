    ...
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      >
        <form class="space-y-4" @submit.prevent="comparePlaylist">
          <label class="space-y-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
            Playlist URL
            <input
              v-model.trim="playListUrl"
              type="url"
              placeholder="https://open.spotify.com/playlist/..."
              class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400"
            />
          </label>
          ...
          <div class="flex flex-wrap items-center gap-4">
            ...
            <button
              type="submit"
              class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
              :disabled="!isFormValid || isSubmitting"
            >
              {{ isSubmitting ? "Comparing..." : "Compare with Local Songs" }}
            </button>
            <span v-if="submitError" class="text-sm text-rose-600">
              {{ submitError }}
            </span>
            <span v-else-if="compareResult" class="text-sm text-emerald-600">
              Found {{ matches.length }} matching track(s).
            </span>
          </div>
        </form>

        <div v-if="compareResult" class="mt-6 border-t border-slate-200 pt-6">
          <h2 class="text-base font-semibold text-slate-900">Matches</h2>
